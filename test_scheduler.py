...
class TestBackup:
    def test_job_flag_filters_to_single_job(self):
        from backup import JOBS
        teslamate_jobs = [j for j in JOBS if j["name"] == "teslamate"]
        assert len(teslamate_jobs) == 1
        assert teslamate_jobs[0]["name"] == "teslamate"

    def test_job_flag_filters_stocks(self):
        from backup import JOBS
        stocks_jobs = [j for j in JOBS if j["name"] == "stocks"]
        assert len(stocks_jobs) == 1
        assert stocks_jobs[0]["name"] == "stocks"

    def test_all_jobs_have_required_fields(self):
        from backup import JOBS
        required = {"name", "dump_cmd", "file_ext", "token_path", "drive_folder_id_env"}
        for job in JOBS:
            missing = required - set(job.keys())
            assert not missing, f"Job '{job['name']}' missing: {missing}"

    def test_unknown_job_name_not_in_jobs(self):
        from backup import JOBS
        matches = [j for j in JOBS if j["name"] == "nonexistent"]
        assert len(matches) == 0

    @patch('backup.run_job', return_value=True)
    @patch('sys.argv', ['backup.py', '--job', 'teslamate'])
    def test_main_runs_single_job(self, mock_run_job):
        from backup import main, JOBS
        main()
        mock_run_job.assert_called_once()
        assert mock_run_job.call_args[0][0]["name"] == "teslamate"

    @patch('backup.run_job', return_value=True)
    @patch('sys.argv', ['backup.py'])
    def test_main_runs_all_jobs_without_flag(self, mock_run_job):
        from backup import main, JOBS
        main()
        assert mock_run_job.call_count == len(JOBS)

    @patch('sys.argv', ['backup.py', '--job', 'nonexistent'])
    def test_main_exits_on_unknown_job(self):
